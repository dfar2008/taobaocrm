...
class Maillisttmps extends CRMEntity {
	...
	* Returns Export Maillisttmps Query.
	*/
	function create_export_query(&$order_by, &$where)
	{
		...
		if(empty($mod_strings)) {
			$mod_strings = return_module_language($current_language,"Maillisttmps");
		}
		$fields_list = $this->getFieldsListFromQuery($sql,$mod_strings);

		$query = "SELECT $fields_list FROM ec_maillisttmps
				LEFT JOIN ec_users
					ON ec_maillisttmps.smownerid = ec_users.id
				LEFT JOIN ec_users as ua
					ON ec_maillisttmps.approvedby = ua.id
				LEFT JOIN ec_users as ucreator
					ON ec_maillisttmps.smcreatorid = ucreator.id
				LEFT JOIN ec_approvestatus ON ec_maillisttmps.approved = ec_approvestatus.statusid ";
		$query .= " left join ec_account ON ec_maillisttmps.accountid=ec_account.accountid  ";
		$query .= " left join ec_contactdetails ON ec_maillisttmps.contact_id=ec_contactdetails.contactid  ";
		$query .= " left join ec_potential ON ec_maillisttmps.potentialid=ec_potential.potentialid  ";
		$query .= " left join ec_salesorder ON ec_maillisttmps.salesorderid=ec_salesorder.salesorderid  ";
		$query .= " left join ec_vendor ON ec_vendor.vendorid=ec_maillisttmps.vendorid  ";
		$query .= " left join ec_purchaseorder ON ec_purchaseorder.purchaseorderid=ec_maillisttmps.purchaseorderid  ";
		...
		for($i=0;$i<$rownum;$i++) {
			...
			$query .= " left join ".$rel_tablename." ON ec_maillisttmps.".$rel_entityid."=".$rel_tablename.".".$rel_entityid;
		}
		$where_auto = " ec_maillisttmps.deleted = 0 ";
	...
	/**
	* Function to get Maillisttmp related Accounts
	* @param  integer   $id      - maillisttmpsid
	...
	*/
	function get_accounts($id)
	{
		...
		if($singlepane_view == 'true')
			$returnset = '&return_module=Maillisttmps&return_action=DetailView&return_id='.$id;
		else
			$returnset = '&return_module=Maillisttmps&return_action=CallRelatedList&return_id='.$id;

		$query = "SELECT ec_account.*,ec_account.accountid as crmid,ec_users.user_name
			FROM ec_account
			INNER JOIN ec_maillisttmps
				ON ec_maillisttmps.maillisttmpsid = ec_account.maillisttmpsid
			WHERE ec_maillisttmps.maillisttmpsid = ".$id."
			AND ec_account.deleted = 0";
		$log->debug("Exiting get_accounts method ...");
		return GetRelatedList('Maillisttmps','Accounts',$focus,$query,$button,$returnset);
	}
	...
	function getReportQuery($is_account=false,$is_contact=false) {
		 $query = "from ec_maillisttmps ";
		 $query .= " left join ec_account as ec_accountMaillisttmps on ec_maillisttmps.accountid = ec_accountMaillisttmps.accountid ";
		 $query .= " left join ec_contactdetails as ec_contactdetailsMaillisttmps on ec_contactdetailsMaillisttmps.contactid = ec_maillisttmps.contact_id  ";
		 $query .= " left join ec_potential as ec_potentialRel ON ec_potentialRel.potentialid=ec_maillisttmps.potentialid  ";
		 $query .= " left join ec_salesorder as ec_salesorderMaillisttmps ON ec_salesorderMaillisttmps.salesorderid=ec_maillisttmps.salesorderid  ";
		 $query .= " left join ec_vendor ec_vendorRel ON ec_vendorRel.vendorid=ec_maillisttmps.vendorid  ";
		 $query .= " left join ec_purchaseorder ec_purchaseorderMaillisttmps ON ec_purchaseorderMaillisttmps.purchaseorderid=ec_maillisttmps.purchaseorderid  ";
		 $query .= " left join ec_users as ec_usersApprove on ec_usersApprove.id = ec_maillisttmps.approvedby ";
		 $query .= " left join ec_users as ec_usersCreator on ec_usersCreator.id = ec_maillisttmps.smcreatorid ";
		 $query .= " left join ec_approvestatus as ec_approvestatus on ec_approvestatus.statusid = ec_crmentityMaillisttmps.approved ";
		 $module = "Maillisttmps";
		 $query_rel = "SELECT ec_entityname.* FROM ec_crmentityrel inner join ec_entityname on ec_entityname.modulename=ec_crmentityrel.relmodule WHERE ec_crmentityrel.module='".$module."'";
		 $fldmod_result = $this->db->query($query_rel);
		 $rownum = $this->db->num_rows($fldmod_result);
		 for($i=0;$i<$rownum;$i++) {
			 ...
			 $rel_entityname = $this->db->query_result($fldmod_result,$i,'fieldname');
			 $rel_entityid = $this->db->query_result($fldmod_result,$i,'entityidfield');
			 $query .= " left join ".$rel_tablename." as ".$rel_tablename."Maillisttmps on ec_maillisttmps.".$rel_entityid."=".$rel_tablename."Maillisttmps.".$rel_entityid;
		 }
		 $query .= " left join ec_users as ec_usersMaillisttmps on ec_usersMaillisttmps.id = ec_maillisttmps.smownerid where ec_maillisttmps.deleted=0 ";
		 return $query;
	}
	function getWorkflowQuery($is_account=false,$is_contact=false) {
		 $query = "select ec_maillisttmps.maillisttmpsid from ec_maillisttmps";
		 $query .= " left join ec_account as ec_accountMaillisttmps on ec_maillisttmps.accountid = ec_accountMaillisttmps.accountid ";
		 $query .= " left join ec_contactdetails as ec_contactdetailsMaillisttmps on ec_contactdetailsMaillisttmps.contactid = ec_maillisttmps.contact_id  ";
		 $query .= " left join ec_potential as ec_potentialRel ON ec_potentialRel.potentialid=ec_maillisttmps.potentialid  ";
		 $query .= " left join ec_salesorder as ec_salesorderMaillisttmps ON ec_salesorderMaillisttmps.salesorderid=ec_maillisttmps.salesorderid  ";
	...
	*/
	...
	{
		/*
		$where_clause = "and ec_maillisttmps.maillisttmpname like '%".trim($this->column_fields['maillisttmpname'])."%'";
		$query = "SELECT * FROM ec_maillisttmps  where deleted=0 $where_clause";
	...
	function saveProductDetails($update_prod_stock=false)
	{
		...
		$query ="update ec_maillisttmps set total='".$_REQUEST['total']."' where maillisttmpsid='".$this->id."'";
		$this->db->query($query);
	...
	function getAssociatedProducts()
	{
		...
		$fieldlist = getProductFieldList("Maillisttmps");
	...
	function getDetailAssociatedProducts()
	{
		...
		$fieldlabellist = getProductFieldLabelList("Maillisttmps");
		$fieldnamelist = getProductFieldList("Maillisttmps");
        $output = "";
	...
	function get_contacts($id)
	{
		...
		return GetRelatedList('Maillisttmps','Contacts',$focus,$query,$button,$returnset);
	}
