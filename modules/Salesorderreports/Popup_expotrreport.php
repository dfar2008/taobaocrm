...
$groupdatearr = array(
	"day"=>"".db_convert("ec_salesorder.trade_created",'date_format',array("'%Y-%m-%d'"),array("'{$startarr[0]}'-'{$startarr[1]}'-DD"))."",
	"week"=>"DATE_FORMAT(ec_salesorder.trade_created, '%x %v')",
	"month"=>"".db_convert('ec_salesorder.trade_created','date_format',array("'%Y-%m'"),array("'{$startarr[0]}-MM'"))."",
	"year"=>"".db_convert("ec_salesorder.trade_created",'date_format',array("'%Y'"),array("'{$startarr[0]}'")).""
);
//统计类型
$grouptype = 'day';
if($_REQUEST['grouptype'] && !empty($_REQUEST['grouptype'])){
