...
class Maillists extends CRMEntity {
	...
	* Returns Export Maillists Query.
	*/
	function create_export_query(&$order_by, &$where)
	{
		...
		if(empty($mod_strings)) {
			$mod_strings = return_module_language($current_language,"Maillists");
		}
		$fields_list = $this->getFieldsListFromQuery($sql,$mod_strings);

		$query = "SELECT $fields_list FROM ec_maillists
				LEFT JOIN ec_users
					ON ec_maillists.smownerid = ec_users.id
				LEFT JOIN ec_users as ua
					ON ec_maillists.approvedby = ua.id
				LEFT JOIN ec_users as ucreator
					ON ec_maillists.smcreatorid = ucreator.id
				LEFT JOIN ec_approvestatus ON ec_maillists.approved = ec_approvestatus.statusid ";
		$query .= " left join ec_account ON ec_maillists.accountid=ec_account.accountid  ";
		$query .= " left join ec_contactdetails ON ec_maillists.contact_id=ec_contactdetails.contactid  ";
		$query .= " left join ec_potential ON ec_maillists.potentialid=ec_potential.potentialid  ";
		$query .= " left join ec_salesorder ON ec_maillists.salesorderid=ec_salesorder.salesorderid  ";
		$query .= " left join ec_vendor ON ec_vendor.vendorid=ec_maillists.vendorid  ";
		$query .= " left join ec_purchaseorder ON ec_purchaseorder.purchaseorderid=ec_maillists.purchaseorderid  ";
		...
		for($i=0;$i<$rownum;$i++) {
			...
			$query .= " left join ".$rel_tablename." ON ec_maillists.".$rel_entityid."=".$rel_tablename.".".$rel_entityid;
		}
		$where_auto = " ec_maillists.deleted = 0 ";
	...
	/**
	* Function to get Maillist related Accounts
	* @param  integer   $id      - maillistsid
	...
	*/
	function get_accounts($id)
	{
		...
		if($singlepane_view == 'true')
			$returnset = '&return_module=Maillists&return_action=DetailView&return_id='.$id;
		else
			$returnset = '&return_module=Maillists&return_action=CallRelatedList&return_id='.$id;

		$query = "SELECT ec_account.*,ec_account.accountid as crmid,ec_users.user_name
			FROM ec_account
			INNER JOIN ec_maillists
				ON ec_maillists.maillistsid = ec_account.maillistsid
			WHERE ec_maillists.maillistsid = ".$id."
			AND ec_account.deleted = 0";
		$log->debug("Exiting get_accounts method ...");
		return GetRelatedList('Maillists','Accounts',$focus,$query,$button,$returnset);
	}
	...
	function getReportQuery($is_account=false,$is_contact=false) {
		 $query = "from ec_maillists ";
		 $query .= " left join ec_account as ec_accountMaillists on ec_maillists.accountid = ec_accountMaillists.accountid ";
		 $query .= " left join ec_contactdetails as ec_contactdetailsMaillists on ec_contactdetailsMaillists.contactid = ec_maillists.contact_id  ";
		 $query .= " left join ec_potential as ec_potentialRel ON ec_potentialRel.potentialid=ec_maillists.potentialid  ";
		 $query .= " left join ec_salesorder as ec_salesorderMaillists ON ec_salesorderMaillists.salesorderid=ec_maillists.salesorderid  ";
		 $query .= " left join ec_vendor ec_vendorRel ON ec_vendorRel.vendorid=ec_maillists.vendorid  ";
		 $query .= " left join ec_purchaseorder ec_purchaseorderMaillists ON ec_purchaseorderMaillists.purchaseorderid=ec_maillists.purchaseorderid  ";
		 $query .= " left join ec_users as ec_usersApprove on ec_usersApprove.id = ec_maillists.approvedby ";
		 $query .= " left join ec_users as ec_usersCreator on ec_usersCreator.id = ec_maillists.smcreatorid ";
		 $query .= " left join ec_approvestatus as ec_approvestatus on ec_approvestatus.statusid = ec_crmentityMaillists.approved ";
		 $module = "Maillists";
		 $query_rel = "SELECT ec_entityname.* FROM ec_crmentityrel inner join ec_entityname on ec_entityname.modulename=ec_crmentityrel.relmodule WHERE ec_crmentityrel.module='".$module."'";
		 $fldmod_result = $this->db->query($query_rel);
		 $rownum = $this->db->num_rows($fldmod_result);
		 for($i=0;$i<$rownum;$i++) {
			 ...
			 $rel_entityname = $this->db->query_result($fldmod_result,$i,'fieldname');
			 $rel_entityid = $this->db->query_result($fldmod_result,$i,'entityidfield');
			 $query .= " left join ".$rel_tablename." as ".$rel_tablename."Maillists on ec_maillists.".$rel_entityid."=".$rel_tablename."Maillists.".$rel_entityid;
		 }
		 $query .= " left join ec_users as ec_usersMaillists on ec_usersMaillists.id = ec_maillists.smownerid where ec_maillists.deleted=0 ";
		 return $query;
	}
	function getWorkflowQuery($is_account=false,$is_contact=false) {
		 $query = "select ec_maillists.maillistsid from ec_maillists";
		 $query .= " left join ec_account as ec_accountMaillists on ec_maillists.accountid = ec_accountMaillists.accountid ";
		 $query .= " left join ec_contactdetails as ec_contactdetailsMaillists on ec_contactdetailsMaillists.contactid = ec_maillists.contact_id  ";
		 $query .= " left join ec_potential as ec_potentialRel ON ec_potentialRel.potentialid=ec_maillists.potentialid  ";
		 $query .= " left join ec_salesorder as ec_salesorderMaillists ON ec_salesorderMaillists.salesorderid=ec_maillists.salesorderid  ";
	...
	/**	Function used to get the sort order for Maillists listview
	 *	@return string	$sorder	- first check the $_REQUEST['sorder'] if request value is empty then check in the $_SESSION['MAILLISTS_SORT_ORDER'] if this session value is empty then default sort order will be returned.
	 */
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (isset($_SESSION['MAILLISTS_SORT_ORDER'])?($_SESSION['MAILLISTS_SORT_ORDER']):($this->default_sort_order));
		$log->debug("Exiting getSortOrder() method ...");
		return $sorder;
	}

	/**	Function used to get the order by value for MAILLISTS listview
	 *	@return string	$order_by  - first check the $_REQUEST['order_by'] if request value is empty then check in the $_SESSION['MAILLISTS_ORDER_BY'] if this session value is empty then default order by will be returned.
	 */
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (isset($_SESSION['MAILLISTS_ORDER_BY'])?($_SESSION['MAILLISTS_ORDER_BY']):($this->default_order_by));
	...
	*/
	...
	{
		/*
		$where_clause = "and ec_maillists.maillistname like '%".trim($this->column_fields['maillistname'])."%'";
		$query = "SELECT * FROM ec_maillists  where deleted=0 $where_clause";
	...
	function saveProductDetails($update_prod_stock=false)
	{
		...
		$query ="update ec_maillists set total='".$_REQUEST['total']."' where maillistsid='".$this->id."'";
		$this->db->query($query);
	...
	function getAssociatedProducts()
	{
		...
		$fieldlist = getProductFieldList("Maillists");
	...
	function get_contacts($id)
	{
		...
		return GetRelatedList('Maillists','Contacts',$focus,$query,$button,$returnset);
	}
