...
$body = eregi_replace("content=([\"|\']?)text/html;([ ]?)charset=([a-z0-9~#%@\&:=?+\/\.,_-]+[a-z0-9~#%@\&=?+\/_-]+)([\"|\']?)", "", $body);
$body = eregi_replace("target=([\"|\']?)[A-Z_]+([\"|\']?)", "target=_blank", $body);
$body = eregi_replace("href=([\"|\']?)file:\/\/[a-z0-9~#%@\&:=?+\/\.,_-]+([\"|\']?)","\\1\\2",$body);
...
if (preg_match("/<body/i", $body))
	$body = eregi_replace("<body", "<body onLoad=\"setHeight()\"", $body);
else
