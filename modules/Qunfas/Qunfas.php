...
class Qunfas extends CRMEntity {
	...
	*/
	function create_export_query(&$order_by, &$where)
	{
		...
		if(empty($mod_strings)) {
			$mod_strings = return_module_language($current_language,"Qunfas");
		}
		$fields_list = $this->getFieldsListFromQuery($sql,$mod_strings);

		$query = "SELECT $fields_list FROM ec_qunfas
				LEFT JOIN ec_users
					ON ec_qunfas.smownerid = ec_users.id
				LEFT JOIN ec_users as ua
					ON ec_qunfas.approvedby = ua.id
				LEFT JOIN ec_users as ucreator
					ON ec_qunfas.smcreatorid = ucreator.id
				LEFT JOIN ec_approvestatus ON ec_qunfas.approved = ec_approvestatus.statusid ";
		$query .= " left join ec_account ON ec_qunfas.accountid=ec_account.accountid  ";
		$query .= " left join ec_contactdetails ON ec_qunfas.contact_id=ec_contactdetails.contactid  ";
		$query .= " left join ec_potential ON ec_qunfas.potentialid=ec_potential.potentialid  ";
		$query .= " left join ec_salesorder ON ec_qunfas.salesorderid=ec_salesorder.salesorderid  ";
		$query .= " left join ec_vendor ON ec_vendor.vendorid=ec_qunfas.vendorid  ";
		$query .= " left join ec_purchaseorder ON ec_purchaseorder.purchaseorderid=ec_qunfas.purchaseorderid  ";
		...
		for($i=0;$i<$rownum;$i++) {
			...
			$query .= " left join ".$rel_tablename." ON ec_qunfas.".$rel_entityid."=".$rel_tablename.".".$rel_entityid;
		}
		$where_auto = " ec_qunfas.deleted = 0 ";
	...
	/**
	* Function to get Qunfa related Accounts
	* @param  integer   $id      - qunfasid
	...
	*/
	function get_accounts($id)
	{
		...
		if($singlepane_view == 'true')
			$returnset = '&return_module=Qunfas&return_action=DetailView&return_id='.$id;
		else
			$returnset = '&return_module=Qunfas&return_action=CallRelatedList&return_id='.$id;

		$query = "SELECT ec_account.*,ec_account.accountid as crmid,ec_users.user_name
			FROM ec_account
			INNER JOIN ec_qunfas
				ON ec_qunfas.qunfasid = ec_account.qunfasid
			WHERE ec_qunfas.qunfasid = ".$id."
			AND ec_account.deleted = 0";
		$log->debug("Exiting get_accounts method ...");
		return GetRelatedList('Qunfas','Accounts',$focus,$query,$button,$returnset);
	}
	...
	function getReportQuery($is_account=false,$is_contact=false) {
		 $query = "from ec_qunfas ";
		 $query .= " left join ec_account as ec_accountQunfas on ec_qunfas.accountid = ec_accountQunfas.accountid ";
		 $query .= " left join ec_contactdetails as ec_contactdetailsQunfas on ec_contactdetailsQunfas.contactid = ec_qunfas.contact_id  ";
		 $query .= " left join ec_potential as ec_potentialRel ON ec_potentialRel.potentialid=ec_qunfas.potentialid  ";
		 $query .= " left join ec_salesorder as ec_salesorderQunfas ON ec_salesorderQunfas.salesorderid=ec_qunfas.salesorderid  ";
		 $query .= " left join ec_vendor ec_vendorRel ON ec_vendorRel.vendorid=ec_qunfas.vendorid  ";
		 $query .= " left join ec_purchaseorder ec_purchaseorderQunfas ON ec_purchaseorderQunfas.purchaseorderid=ec_qunfas.purchaseorderid  ";
		 $query .= " left join ec_users as ec_usersApprove on ec_usersApprove.id = ec_qunfas.approvedby ";
		 $query .= " left join ec_users as ec_usersCreator on ec_usersCreator.id = ec_qunfas.smcreatorid ";
		 $query .= " left join ec_approvestatus as ec_approvestatus on ec_approvestatus.statusid = ec_crmentityQunfas.approved ";
		 $module = "Qunfas";
		 $query_rel = "SELECT ec_entityname.* FROM ec_crmentityrel inner join ec_entityname on ec_entityname.modulename=ec_crmentityrel.relmodule WHERE ec_crmentityrel.module='".$module."'";
		 $fldmod_result = $this->db->query($query_rel);
		 $rownum = $this->db->num_rows($fldmod_result);
		 for($i=0;$i<$rownum;$i++) {
			 ...
			 $rel_entityname = $this->db->query_result($fldmod_result,$i,'fieldname');
			 $rel_entityid = $this->db->query_result($fldmod_result,$i,'entityidfield');
			 $query .= " left join ".$rel_tablename." as ".$rel_tablename."Qunfas on ec_qunfas.".$rel_entityid."=".$rel_tablename."Qunfas.".$rel_entityid;
		 }
		 $query .= " left join ec_users as ec_usersQunfas on ec_usersQunfas.id = ec_qunfas.smownerid where ec_qunfas.deleted=0 ";
		 return $query;
	}
	function getWorkflowQuery($is_account=false,$is_contact=false) {
		 $query = "select ec_qunfas.qunfasid from ec_qunfas";
		 $query .= " left join ec_account as ec_accountQunfas on ec_qunfas.accountid = ec_accountQunfas.accountid ";
		 $query .= " left join ec_contactdetails as ec_contactdetailsQunfas on ec_contactdetailsQunfas.contactid = ec_qunfas.contact_id  ";
		 $query .= " left join ec_potential as ec_potentialRel ON ec_potentialRel.potentialid=ec_qunfas.potentialid  ";
		 $query .= " left join ec_salesorder as ec_salesorderQunfas ON ec_salesorderQunfas.salesorderid=ec_qunfas.salesorderid  ";
	...
	*/
	...
	{
		/*
		$where_clause = "and ec_qunfas.qunfaname like '%".trim($this->column_fields['qunfaname'])."%'";
		$query = "SELECT * FROM ec_qunfas  where deleted=0 $where_clause";
	...
	function saveProductDetails($update_prod_stock=false)
	{
		...
		$query ="update ec_qunfas set total='".$_REQUEST['total']."' where qunfasid='".$this->id."'";
		$this->db->query($query);
	...
	function getAssociatedProducts()
	{
		...
		$fieldlist = getProductFieldList("Qunfas");
	...
	function getDetailAssociatedProducts()
	{
		...
		$fieldlabellist = getProductFieldLabelList("Qunfas");
		$fieldnamelist = getProductFieldList("Qunfas");
        $output = "";
	...
	function get_contacts($id)
	{
		...
		return GetRelatedList('Qunfas','Contacts',$focus,$query,$button,$returnset);
	}
