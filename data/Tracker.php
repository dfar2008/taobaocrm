...
//No genius required. Just add an if case and change the query so that it puts the tracker entry whenever you touch on the DetailView of the required entity
	 ...
	 if($current_module != '')
	 {
		 ...
		 if(strlen($item_summary) > 30)
	     {
          ...
          $query = "INSERT into $this->table_name (id,user_id, module_name, item_id, item_summary) values ('".$id."','$user_id', '$current_module', '$esc_item_id', ".$this->db->formatString($this->table_name,'item_summary',$item_summary).")";
