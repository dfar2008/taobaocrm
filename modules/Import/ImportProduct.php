    ...
    */
	...
	function save($module) {
		...
		if(isset($_SESSION['import_overwrite2']) && $_SESSION['import_overwrite2'] == 1) {
			if(!empty($this->column_fields['productcode'])) {
				$where_clause = " and ec_products.productcode='".trim($this->column_fields['productcode'])."' ";
	...
	function saveInventPro(){
		...
		$comment2=$this->column_fields['comment2'];
		if($comment2 !=''){
			$comment .= ",".$comment2;
		}
		$salesorderid ='';
		$where_clause = "and ec_salesorder.oid='".trim($oid)."'";
