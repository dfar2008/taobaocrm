	...
	{foreach key=mainlabel item=maindata from=$subdata}
		...
		{elseif $uitype eq 56}
			...
			{if $fldname eq 'notime' && $ACTIVITY_MODE eq 'Events'}
				{if $fldvalue eq 1}
					<td width="30%" align=left class="dvtCellInfo">
						<input{$disable} name="{$fldname}" type="checkbox" tabindex="{$vt_tab}" onclick="toggleTime()" checked class="upaccount">
					</td>
				{else}
					<td width="30%" align=left class="dvtCellInfo">
			...
			{else}
				{if $fldvalue eq 1}
					<td width="30%" align=left class="dvtCellInfo">
						<input{$disable} name="{$fldname}" type="checkbox" tabindex="{$vt_tab}" checked class="upaccount">
					</td>
				{else}
					<td width="30%" align=left class="dvtCellInfo">
		...
		{elseif $uitype eq 23 || $uitype eq 5 || $uitype eq 6}
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				<input{$disable} name="{$fldname}" tabindex="{$vt_tab}" id="jscal_field_{$fldname}" type="text" style="border:1px solid #bababa;" size="11" maxlength="10" value="{$date_val}" class="upaccount">
				<img src="{$IMAGE_PATH}calendar.gif" id="jscal_trigger_{$fldname}">

				{if $uitype eq 6}
					<input{$disable} name="time_start" tabindex="{$vt_tab}" style="border:1px solid #bababa;" size="5" maxlength="5" type="text" value="{$time_val}" class="upaccount">
				{/if}
				{if $uitype eq 23}
					<input{$disable} name="time_end" tabindex="{$vt_tab}" style="border:1px solid #bababa;" size="5" maxlength="5" type="text" value="{$time_val}" class="upaccount">
				{/if}
		...
		{elseif $uitype eq 68 || $uitype eq 66 || $uitype eq 62}
			<td width="20%" class="dvtCellLabel" align=right>
				<select{$disable} class="small upaccount" name="parent_type" onChange='document.EditView.parent_name.value=""; document.EditView.parent_id.value=""'>
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				<input{$disable} name="parent_name" readonly type="text" style="border:1px solid #bababa;" value="{$fldvalue}" class="upaccount">
		...
		{elseif $uitype eq 357}
			...
			<td width="90%" colspan="3">
				<input{$disable} name="{$fldname}" type="hidden" value="{$secondvalue}" class="upaccount">
				<textarea{$disable} readonly name="parent_name" cols="70" rows="2" class="upaccount"> {$fldvalue}</textarea>&nbsp;
				<select{$disable} name="parent_type" class="small upaccount">
		   ...
		   <tr style="height:25px">
			...
			<td width="30%" align=left class="dvtCellInfo">
				<input{$disable} name="ccmail" type="text" class="detailedViewTextBox upaccount"value="">
			</td>
			<td width="20%" class="dvtCellLabel" align=right>BCC:&nbsp;</td>
			<td width="30%" align=left class="dvtCellInfo">
				<input{$disable} name="bccmail" type="text" class="detailedViewTextBox upaccount"value="">
			</td>
		   </tr>
		...
		{elseif $uitype eq 69}
			...
			<td colspan="3" width="30%" align=left class="dvtCellInfo">
				{if $MODULE eq 'Products'}
					<input{$disable} name="del_file_list" type="hidden" value="" class="upaccount">
					<div id="files_list" style="border: 1px solid grey; width: 500px; padding: 5px; background: rgb(255, 255, 255) none repeat scroll 0%; -moz-background-clip: initial; -moz-background-origin: initial; -moz-background-inline-policy: initial; font-size: x-small">Files Maximum 6
						<input{$disable} id="my_file_element" type="file" name="file_1" tabindex="{$vt_tab}" class="upaccount" >
		...
		{elseif $uitype eq 98}<!-- Role Selection Popup -->		
			...
			<td width="30%" align=left class="dvtCellInfo">
			{if $thirdvalue eq 1}
				<input{$disable} name="role_name" id="role_name" readonly class="txtBox upaccount" tabindex="{$vt_tab}" value="{$secondvalue}" type="text">&nbsp;
				<a href="javascript:openPopup();"><img src="{$IMAGE_PATH}select.gif" align="absmiddle" border="0"></a>
			{else}	
				<input{$disable} name="role_name" id="role_name" tabindex="{$vt_tab}" class="txtBox upaccount" readonly value="{$secondvalue}" type="text">&nbsp;
			{/if}	
			<input{$disable} name="user_role" id="user_role" value="{$fldvalue}" type="hidden" class="upaccount">
			</td>
		{elseif $uitype eq 104}<!-- Mandatory Email Fields -->			
			...
			{elseif $uitype eq 101}<!-- for reportsto field USERS POPUP -->
				...
				<td width="30%" align=left class="dvtCellInfo">
				<input{$disable} readonly name='reports_to_name' class="small upaccount" type="text" value='{$fldvalue}' tabindex="{$vt_tab}" ><input{$disable} name='reports_to_id' type="hidden" value='{$secondvalue}'  class="upaccount">&nbsp;<input{$disable} title="Change [Alt+C]" accessKey="C" type="button" class="small" value='{$UMOD.LBL_CHANGE}' name=btn1 LANGUAGE=javascript onclick='return window.open("index.php?module=Users&action=Popup&form=UsersEditView&form_submit=false","test","width=640,height=522,resizable=0,scrollbars=0");'>
	            </td>
			...
			<td width="30%" align=left class="dvtCellInfo">
			   {if $secondvalue eq 1}
			   	<select{$disable} name="{$fldname}" tabindex="{$vt_tab}" class="small upaccount">
			   {else}
			   	<select{$disable} disabled name="{$fldname}" tabindex="{$vt_tab}" class="small upaccount">
			   {/if} 
			...
			{elseif $uitype eq 99}
				...
				<td width=30% align=left class="dvtCellInfo">
					<input{$disable} type="password" name="{$fldname}" tabindex="{$vt_tab}" value="{$fldvalue}" class="detailedViewTextBox upaccount">
				</td>
				{/if}
		{elseif $uitype eq 30}
			<td width="20%" class="dvtCellLabel" align=right>
			...
			<td colspan="3" width="30%" align=left class="dvtCellInfo">
				...
				<input{$disable} type="radio" name="set_reminder" tabindex="{$vt_tab}" value="Yes" {$check}  class="upaccount">&nbsp;{$yes_val}&nbsp;
