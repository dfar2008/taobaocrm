...
<div id="sharerecorddiv" style="display:none;position:absolute;width:250px;" class="layerPopup"></div>
<script>
	function callSharerecordDiv(id)
	{ldelim}
		new Ajax.Request(
			'index.php',
			{ldelim}queue: {ldelim}position: 'end', scope: 'command'{rdelim},
				method: 'post',
				postBody: 'module={$MODULE}&action={$MODULE}Ajax&file=ShareRecord&record='+id,
				onComplete: function(response) {ldelim}				        
					$("sharerecorddiv").innerHTML=response.responseText;
	...
	{rdelim}
	function getShareuserids() {ldelim}
		var shareuserids = "";
		for(var i=0;i<document.sharerecordform.elements.length;i++) {ldelim}
		    if(document.sharerecordform.elements[i].type == 'checkbox' && document.sharerecordform.elements[i].checked) {ldelim}
			shareuserids = shareuserids + document.sharerecordform.elements[i].value + ",";
		    {rdelim}
		{rdelim}
		document.sharerecordform.shareuserids.value=shareuserids;
	{rdelim}
			...
			{if $SinglePane_View eq 'false'}
		           ...
		           <table border=0 cellspacing=0 cellpadding=0 width=100%>
				     {strip}<tr nowrap>
						...
						{if $EDIT eq 'permitted'}
						<input title="{$APP.LBL_EDIT_BUTTON_TITLE}" accessKey="{$APP.LBL_EDIT_BUTTON_KEY}" class="crmbutton small edit" onclick="this.form.return_module.value='{$MODULE}'; this.form.return_action.value='DetailView'; this.form.return_id.value='{$ID}';this.form.module.value='{$MODULE}';this.form.action.value='EditView'" type="submit" name="Edit" value="&nbsp;{$APP.LBL_EDIT_BUTTON_LABEL}&nbsp;">&nbsp;
						<input title="{$APP.LBL_SHARE_BUTTON_LABEL}" class="crmbutton small edit" onclick="fnvshobj(this,'sharerecorddiv');callSharerecordDiv({$ID})" type="button" name="Share" value="&nbsp;{$APP.LBL_SHARE_BUTTON_LABEL}&nbsp;">&nbsp;
						{/if}
