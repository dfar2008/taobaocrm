					...
					<td style="padding:10px;" >
						...
						<tr>
							...
							<td width="30%" class="dvtCellLabel">
								...
								<input type="hidden" name="action" value="PopupForKucun">
			...
			<table border="0" cellpadding="0" cellspacing="0" style="padding-right:5px;padding-top:2px;padding-bottom:2px;">
			  ...
			  {/foreach}
		       ...
		       <tr><td valign="top" width="15%">
		               {include file="Products/CatalogPopupForKucun.tpl"}
		       </td>
		       <td width=70% align="left" valign=top style="border-left:2px dashed #cccccc;padding:13px">
			<div id="ListViewContents">
				{include file="Products/PopupContentsForKucun.tpl"}
			</div>
			</td>
			</tr>
			</table>
...
{ldelim}
	...
	urlstring = urlstring +'&query=true&file=PopupForKucun&module=Products&action=ProductsAjax&ajax=true';
...
{ldelim}
    ...
    var urlstring ="module="+module+"&action="+module+"Ajax&file=PopupForKucun&ajax=true&"+url;
    urlstring +=gethiddenelements();
    new Ajax.Request(
                'index.php',
                {ldelim}queue: {ldelim}position: 'end', scope: 'command'{rdelim},
				...
				*/
				...
				for (var j = 0; j < id_arr.length; j++) {
					...
					if(!repeated) {
						...
						var serial_no = $("serial_no_"+idvalue).value;
						qtyinstock = $("qtyinstock_"+idvalue).value;
						if(productstring != "") {
							productstring = productstring + ";" + idvalue + "##" + productname + "##" + productcode + "##" + listprice + "##" + qtyinstock + "##" + serial_no;
						} else {
							productstring = idvalue + "##" + productname + "##" + productcode + "##" + listprice + "##" + qtyinstock + "##" + serial_no;
						}
					}
				}
				...
				productvalue = idvalue + "##" + productname + "##" + productcode + "##" + listprice + "##" + qtyinstock  + "##" + serial_no + ";";
...
function addMultiProductRow()
{
	...
	for (var j = 0; j < id_arr.length; j++) {
		if(id_arr[j] != "")
		{
			...
			//format:productid#productname#productcode#listprice#qtyinstock#serial_no
			...
			colfour.innerHTML= serial_no+'<input id="serial_no'+count+'" name="serial_no'+count+'" value="'+ serial_no +'" type="hidden">';
			
			//Quantity
			colfive.className = "crmTableRow small";
			colfive.innerHTML='<input id="qty'+count+'" name="qty'+count+'" type="text" class="small " style="width:50px" onfocus="this.className=\'detailedViewTextBoxOn\'" onBlur="FindDuplicate(); settotalnoofrows();calcGrandTotal();" value=""/>';

			//comments
			colsix.className = "crmTableRow small";
			colsix.innerHTML='<input id="comment'+count+'" name="comment'+count+'" class=small style="width:200px">';
