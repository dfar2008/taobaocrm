...
require_once($root_directory.'include/utils/ChineseUtils.php');
/**
...
function checkDiscuzUsername($username) {
    include_once("config.php");
    if($is_discuz) {
        $link = mysql_connect($discuz['db_host_name'],$discuz['db_username'],$discuz['db_password']) or die('Could not connect: ' . mysql_error());
        mysql_select_db($discuz['db_name']) or die('Could not select database');

        // Performing SQL query
        $query = "SELECT username FROM cdb_members where username='".trim($username)."'";
...
function getEntityTable($modulename) {
	...
	if(!$entityArr) {
		...
		if($rownum > 0) {
			$entityArr = $adb->fetch_array($result);
		}
...
function getPickListWithField($fieldname,$fieldvalue = "")
{
	...
	if(!$picklist_array) {
		...
		while($row = $adb->fetchByAssoc($pickListResult)) {
