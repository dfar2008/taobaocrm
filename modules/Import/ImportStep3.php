...
require_once('modules/Import/ImportVendor.php');
require_once('modules/Import/ImportVcontact.php');
...
if ( isset( $_REQUEST['overwrite']) && $_REQUEST['overwrite'] == 'on')
{
	$overwrite = 1;
}
...
//Constructing the custom ec_field Array
require_once('include/CustomFieldUtil.php');
$custFldArray = getCustomFieldArray($_REQUEST['module']);
//Initializing  an empty Array to store the custom ec_field Column Name and Value
...
foreach ($_REQUEST as $name=>$value)
{
	...
	// this value is a user defined ec_field name
	$user_field = $value;

	// pull out the column position for this ec_field name
	$pos = substr($name,6);

	// make sure we haven't seen this ec_field defined yet
...
//Mark the last imported records as deleted which are imported by the current user in ec_users_last_import ec_table
