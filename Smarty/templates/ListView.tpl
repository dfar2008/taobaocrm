      ...
      <tbody>
        <tr>
              <td style="padding-left:5px;">
                 <input title="{$APP.LBL_NEW_BUTTON_LABEL}{$APP.$MODULE}" accessKey="{$APP.LNK_NEW_NOTE}" class="crmbutton small create" onclick="javascript:location.href='index.php?module={$MODULE}&action=EditView'" type="button" name="Create" value="{$APP.LBL_NEW_BUTTON_LABEL}{$APP.$MODULE}">&nbsp;
               </td> 
...
<tbody><tr><td style="padding:5px;">
	...
	<table style="background-color: rgb(234, 234, 234);" class="small" width="100%" border="0" cellpadding="3" cellspacing="1">
      ...
      {if $relorderinfo != ''}
       ...
       {foreach item=orderinfos key=key from=$relorderinfo}
          {foreach item=orderinfo key=key from=$orderinfos}
          <tr bgcolor="white">
