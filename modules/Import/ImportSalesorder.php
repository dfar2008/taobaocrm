...
class ImportSalesorder extends SalesOrder {
	...
	function save($module) {
		...
		$this->log->info("salesorder save function(import_overwrite)");
			if(!empty($this->column_fields['oid'])) {
				$where_clause = "and ec_salesorder.oid='".trim($this->column_fields['oid'])."'";
				$query = "SELECT ec_salesorder.salesorderid FROM ec_salesorder where deleted=0 $where_clause";
	...
	function SaveSalesOrder($salesorderid,$column_fields){
		...
		$column_fields['subject'] = $salesorder_seqprefix.date("Ymd")."-".$this->get_next_id();
		...
		if($column_fields['orderstatus'] == '交易成功'){
			$pay_str = ",allsuccessbuy=allsuccessbuy+1";
		}else{
			$pay_str='';
		}
		if(!empty($salesorderid)){
			if($keyvals !=''){
				$updatesql  = "update ec_salesorder set {$keyvals} where salesorderid= $salesorderid";
				$this->db->query($updatesql);
				
				$updatesql_acc = "update ec_account set ordertotal=ordertotal+".$column_fields['total'].",buy_pro_num=buy_pro_num+".$column_fields['num']."".$pay_str.",lastorderdate='".$column_fields['createdtime']."' where accountid= ".$column_fields['account_id']." ";
				//echo $updatesql_acc;echo "<br>";
			...
			if($fieldkeys !=''){
				...
				$updatesql_acc = "update ec_account set ordernum=ordernum+1,ordertotal=ordertotal+".$column_fields['total'].",buy_pro_num=buy_pro_num+".$column_fields['num']."".$pay_str.",lastorderdate='".$column_fields['createdtime']."'  where accountid= ".$column_fields['account_id']." ";
				$this->db->query($updatesql_acc);
