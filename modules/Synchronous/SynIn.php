	...
	foreach($orders as $order){  //order details begin
		...
		if(empty($productid_tmp)){
			...
			if(empty($ItemInfo)){
				...
				$insertproductsql = "insert into ec_products(productid,productname,productcode,detail_url,num_iid,num,price,smcreatorid,smownerid,modifiedby,createdtime,modifiedtime,deleted) values(".$productid.",'".$productname."','".$productcode."','".$detail_url ."','".$num_iid."','".$pro_num."','".$price."',".$current_user->id.",".$current_user->id.",0,'".$createdtime_pro."','".$modifiedtime_pro."',0)";
