...
class OrdernumreportsDataPopulator extends CRMEntity {
	...
	function create_defautdata($is_upgrade=false) {
		...
		$query = "select tabid from ec_tab where name='Ordernumreports'";
		...
		if($noofrows > 0) {
			...
			$this->db->query("insert into ec_tab values (".$tab_id.",'Ordernumreports',0,".$tab_id.",'Ordernumreports',null,null,1,0)");
