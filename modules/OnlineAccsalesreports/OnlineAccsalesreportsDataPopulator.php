...
class OnlineAccsalesreportsDataPopulator extends CRMEntity {
	...
	function create_defautdata($is_upgrade=false) {
		...
		$query = "select tabid from ec_tab where name='OnlineAccsalesreports'";
		...
		if($noofrows > 0) {
			...
			$this->db->query("insert into ec_tab values (".$tab_id.",'OnlineAccsalesreports',0,".$tab_id.",'OnlineAccsalesreports',null,null,1,0)");
