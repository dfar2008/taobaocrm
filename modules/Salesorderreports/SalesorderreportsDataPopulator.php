...
class SalesorderreportsDataPopulator extends CRMEntity {
		
	function SalesorderreportsDataPopulator() {
		$this->log = LoggerManager::getLogger('SalesorderreportsDataPopulator');
	...
	function create_defautdata($is_upgrade=false) {
		...
		$query = "select tabid from ec_tab where name='Salesorderreports'";
		...
		if($noofrows > 0) {
			...
			$this->db->query("insert into ec_tab values (".$tab_id.",'Salesorderreports',0,".$tab_id.",'Salesorderreports',null,null,1,0)");
