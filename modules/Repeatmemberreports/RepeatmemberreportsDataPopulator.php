...
class RepeatmemberreportsDataPopulator extends CRMEntity {
		
	function RepeatmemberreportsDataPopulator() {
		$this->log = LoggerManager::getLogger('RepeatmemberreportsDataPopulator');
	...
	function create_defautdata($is_upgrade=false) {
		...
		$query = "select tabid from ec_tab where name='Repeatmemberreports'";
		...
		if($noofrows > 0) {
			...
			$this->db->query("insert into ec_tab values (".$tab_id.",'Repeatmemberreports',0,".$tab_id.",'Repeatmemberreports',null,null,1,0)");
