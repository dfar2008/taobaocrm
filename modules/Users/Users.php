...
/*********************************************************************************
 * $Header: /advent/projects/wesat/ec_crm/sugarcrm/modules/Users/Users.php,v 1.10 2005/04/19 14:40:48 ray Exp $
...
class Users {
	...
	var $tab_name = Array('ec_users','ec_attachments','ec_user2role');	
	var $tab_name_index = Array('ec_users'=>'id','ec_attachments'=>'attachmentsid','ec_user2role'=>'userid');
	...
	var $defhomeview;
	//var $homeorder_array = array('ALVT','HDB','PLVT','QLTQ','CVLVT','HLT','OLV','GRT','OLTSO','ILTI','MNL','OLTPO','LTFAQ','Notes','Charges','Gathers','Expenses','Documents');
	var $homeorder_array = array('CVLVT','HDB','ALVT','PLVT','QLTQ','HLT','OLV','OLTSO','ILTI','OLTPO','LTFAQ','Notes','Charges','Gathers','Expenses','Documents');

	//var $encodeFields = Array("first_name", "last_name", "description");
	//changed by dingjianting on 2007-1-11 for chinese name
	...
	var $list_fields = Array(
		//changed by dingjianting on 2007-1-11 for chinese name
		//'First Name'=>Array('ec_users'=>'first_name'),
		'Last Name'=>Array('ec_users'=>'last_name'),
		'Role Name'=>Array('ec_user2role'=>'roleid'),
...
//		$salt = $dbconfig['db_name'];
	...
	function load_user($user_password)
	{
		...
		$user_preferences = $this->db->query_result($result,0,"user_preferences");
		...
		$this->loadPreferencesFromDB($user_preferences);
	...
	{		
		global $mod_strings;
		global $current_user;
		$this->log->debug("Starting password change for admin");
		$encrypted_new_password = $this->encrypt_password($new_password);
		$user_hash = strtolower(md5($new_password));
	...
	function verify_data()
	{
		...
		$query = "SELECT user_name from ec_users where user_name='$usr_name' AND id<>'$this->id' AND deleted=0";
		$result =$this->db->query($query, true, "Error selecting possible duplicate users: ");
		$dup_users = $this->db->fetchByAssoc($result);

		$query = "SELECT user_name from ec_users where is_admin = 'on' AND deleted=0";
		$result =$this->db->query($query, true, "Error selecting possible duplicate ec_users: ");
	...
	{
		$query = "select user_name from ec_users where id={$userid}";
		$result = $this->db->query($query);
...
//		$this->column_fields["groupid"] = $groupid;
	...
	function getListQuery($where=""){		
		$query = "select id,last_name,user_name,ec_role.roleid,ec_role.rolename,ec_groups.groupid,ec_groups.groupname,status,is_admin,email1,phone_work from ec_users left join ec_user2role on ec_user2role.userid=ec_users.id left join ec_role on ec_role.roleid=ec_user2role.roleid left join ec_users2group on ec_users2group.userid=ec_users.id left join ec_groups on ec_groups.groupid=ec_users2group.groupid where deleted=0 ";
		$query = $query.$where;
		return $query;
	}
