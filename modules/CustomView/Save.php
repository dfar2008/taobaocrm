...
if($cvmodule != "")
{
	...
	if(!$cvid)
	{
		...
		if($genCVid != "")
		{
			...
			$customviewsql = "INSERT INTO ec_customview(cvid, viewname,	setdefault, setmetrics,	entitytype,setpublic,collectcolumn,smownerid)	VALUES (".$genCVid.",".$adb->quote($viewname).",".$setdefault.",".$setmetrics.",".$adb->quote($cvmodule).",".$adb->quote($setpublic).",'$choosecollectcolhtml',".$current_user->id.")";
	...
	else
	{
		...
		$updatecvsql = "UPDATE ec_customview SET viewname = ".$adb->quote($viewname).",setdefault = ".$setdefault.",setpublic = ".$adb->quote($setpublic).",setmetrics = ".$setmetrics.",collectcolumn='$choosecollectcolhtml' WHERE cvid = ".$cvid;
