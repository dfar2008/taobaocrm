 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header: /cvsroot/vtigercrm/ec_crm/include/utils/ListViewUtils.php,v 1.32 2006/02/03 06:53:08 mangai Exp $
...
function getValue($uitype, $list_result,$fieldname,$focus,$module,$entity_id,$list_result_count,$mode,$popuptype,$returnset='',$viewid='')
{
	...
	elseif($uitype == 1007) //display approve status in detailview page
	{
		$temp_val = $adb->query_result($list_result,$list_result_count,$fieldname);
	...
	$log->debug("Entering getListQuery() method ...");
		...
		{	
			...
			$query = "SELECT  ec_account.accountid as crmid,ec_users.user_name,ec_account.*
				...
				WHERE ec_account.deleted = 0 and ec_users.id = '".$_SESSION['authenticated_user_id']."' ";
		...
		elseif($module == "Notes")
		{
			$query = "SELECT ec_notes.notesid as crmid,ec_users.user_name,ec_notes.*
				...
				WHERE ec_notes.deleted = 0 and ec_users.id = '".$_SESSION['authenticated_user_id']."' ";
		...
		elseif($module == "Products")
		{
			...
			$query = "SELECT ec_products.productid as crmid, ec_products.*
			    ...
			    WHERE ec_products.deleted = 0 and ec_users.id = '".$_SESSION['authenticated_user_id']."' ";
		...
		elseif($module == "SalesOrder")
		{
			//Query modified to sort by assigned to
				...
				WHERE ec_salesorder.deleted = 0 and ec_users.id = '".$_SESSION['authenticated_user_id']."' ";
