...
class Homestuff
{
	...
	Private function getModuleFilters($sid)
	{
		...
		$cvid_check_query = $adb->pquery("SELECT * FROM ec_customview WHERE cvid = ?",array($cvid));
			...
			if($adb->num_rows($cvid_check_query)>0)
			{
				...
				for($l=0;$l < $column_count;$l++)
				{
					...
					if($fld_permission == 0 && $adb->num_rows($cv_presence)){ 
						$field_query = $adb->pquery("SELECT fieldlabel FROM ec_field WHERE fieldname = ? AND tablename = ?", array($fldname,$tabname));
