...
function previewRelmodTable(nowmodule)
{
    ...
    new Ajax.Request(
			'index.php',
				...
				method: 'post',
				postBody: 'module=Settings&action=SettingsAjax&file=createRelmodTable&nowmodule='+nowmodule,
...
<tbody><tr>
	...
	{include file='SetMenu.tpl'}
		...
		{if $MODE neq 'view'}
			<input type="hidden" name="action" value="UpdateRelmodField">
		{else}
			<input type="hidden" name="action" value="EditRelmodField">
		{/if}	
		<tr>
			...
			<td colspan=2 class=heading2 valign=bottom><b><a href="index.php?module=Settings&action=index&parenttab=Settings">{$MOD.LBL_SETTINGS}</a> > 模块字段管理 </b></td>
			<td rowspan=2 class="small" align=right>&nbsp;</td>
		</tr>
		<tr>
			<td valign=top class="small">定制各模块被关联时在相关信息中显示的字段</td>
		</tr>
		</table>
		<br>
		<table border=0 cellspacing=0 cellpadding=5 width=100% class="tableHeading">
		<tr>
			<td class="big"><strong>相关信息中模块字段管理</strong></td>
			<td class="small" align=right>
			{if $MODE neq 'edit'}
				<input name="Edit" type="submit" class="crmButton small edit" value="{$APP.LBL_EDIT_BUTTON}" />
                                <input class="crmbutton small edit" onclick="previewRelmodTable('{$DEF_MODULE}')" name="previewproduct" value="预览" type="button"/>
