...
class NewmemberreportsDataPopulator extends CRMEntity {
	...
	function create_defautdata($is_upgrade=false) {
		...
		$query = "select tabid from ec_tab where name='Newmemberreports'";
		...
		if($noofrows > 0) {
			...
			$this->db->query("insert into ec_tab values (".$tab_id.",'Newmemberreports',0,".$tab_id.",'Newmemberreports',null,null,1,0)");
