...
class Relsettings extends CRMEntity {
	...
	function save_module($module)
	{
		...
		if(isset($module_enable_product) && $module_enable_product && $_REQUEST['action'] != 'RelsettingsAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW')
		{
	...
	*/
	...
	function getListQuery($where,$isSearchAll=false){
		...
		$query = "SELECT ec_relsettings.relsettingsid as crmid,ec_users.user_name,
		ec_relsettings.* FROM ec_relsettings
		LEFT JOIN ec_users
			ON ec_users.id = ec_relsettings.smownerid ";
		
		$query .= " WHERE ec_relsettings.deleted = 0 ";

		return $query;
	}

	/**
	* Function to get Relsetting related Accounts
	* @param  integer   $id      - relsettingsid
	...
	*/
	function get_accounts($id)
	{
		...
		if($singlepane_view == 'true')
			$returnset = '&return_module=Relsettings&return_action=DetailView&return_id='.$id;
		else
			$returnset = '&return_module=Relsettings&return_action=CallRelatedList&return_id='.$id;

		$query = "SELECT ec_account.*,ec_account.accountid as crmid,ec_users.user_name
			FROM ec_account
			INNER JOIN ec_relsettings
				ON ec_relsettings.relsettingsid = ec_account.relsettingsid
			WHERE ec_relsettings.relsettingsid = ".$id."
			AND ec_account.deleted = 0";
		$log->debug("Exiting get_accounts method ...");
		return GetRelatedList('Relsettings','Accounts',$focus,$query,$button,$returnset);
	}

	//get next salesorder id
	function get_next_id() {
		//$query = "select count(*) as num from ec_relsettings";
		//$result = $this->db->query($query);
		//$num = $this->db->query_result($result,0,'num') + 1;
		$num = $this->db->getUniqueID("ec_relsettings");
	...
	function getReportQuery($is_account=false,$is_contact=false) {
		 $query = "from ec_relsettings ";
		 $query .= " left join ec_account as ec_accountRelsettings on ec_relsettings.accountid = ec_accountRelsettings.accountid ";
		 $query .= " left join ec_contactdetails as ec_contactdetailsRelsettings on ec_contactdetailsRelsettings.contactid = ec_relsettings.contact_id  ";
		 $query .= " left join ec_potential as ec_potentialRel ON ec_potentialRel.potentialid=ec_relsettings.potentialid  ";
		 $query .= " left join ec_salesorder as ec_salesorderRelsettings ON ec_salesorderRelsettings.salesorderid=ec_relsettings.salesorderid  ";
		 $query .= " left join ec_vendor ec_vendorRel ON ec_vendorRel.vendorid=ec_relsettings.vendorid  ";
		 $query .= " left join ec_purchaseorder ec_purchaseorderRelsettings ON ec_purchaseorderRelsettings.purchaseorderid=ec_relsettings.purchaseorderid  ";
		 $query .= " left join ec_users as ec_usersApprove on ec_usersApprove.id = ec_relsettings.approvedby ";
		 $query .= " left join ec_users as ec_usersCreator on ec_usersCreator.id = ec_relsettings.smcreatorid ";
		 $query .= " left join ec_approvestatus as ec_approvestatus on ec_approvestatus.statusid = ec_crmentityRelsettings.approved ";
		 $module = "Relsettings";
		 $query_rel = "SELECT ec_entityname.* FROM ec_crmentityrel inner join ec_entityname on ec_entityname.modulename=ec_crmentityrel.relmodule WHERE ec_crmentityrel.module='".$module."'";
		 $fldmod_result = $this->db->query($query_rel);
		 $rownum = $this->db->num_rows($fldmod_result);
		 for($i=0;$i<$rownum;$i++) {
			 ...
			 $rel_entityname = $this->db->query_result($fldmod_result,$i,'fieldname');
			 $rel_entityid = $this->db->query_result($fldmod_result,$i,'entityidfield');
			 $query .= " left join ".$rel_tablename." as ".$rel_tablename."Relsettings on ec_relsettings.".$rel_entityid."=".$rel_tablename."Relsettings.".$rel_entityid;
		 }
		 $query .= " left join ec_users as ec_usersRelsettings on ec_usersRelsettings.id = ec_relsettings.smownerid where ec_relsettings.deleted=0 ";
		 return $query;
	}
	function getWorkflowQuery($is_account=false,$is_contact=false) {
		 $query = "select ec_relsettings.relsettingsid from ec_relsettings";
		 $query .= " left join ec_account as ec_accountRelsettings on ec_relsettings.accountid = ec_accountRelsettings.accountid ";
		 $query .= " left join ec_contactdetails as ec_contactdetailsRelsettings on ec_contactdetailsRelsettings.contactid = ec_relsettings.contact_id  ";
		 $query .= " left join ec_potential as ec_potentialRel ON ec_potentialRel.potentialid=ec_relsettings.potentialid  ";
		 $query .= " left join ec_salesorder as ec_salesorderRelsettings ON ec_salesorderRelsettings.salesorderid=ec_relsettings.salesorderid  ";
	...
	/**	Function used to get the sort order for Relsettings listview
	 *	@return string	$sorder	- first check the $_REQUEST['sorder'] if request value is empty then check in the $_SESSION['RELSETTINGS_SORT_ORDER'] if this session value is empty then default sort order will be returned.
	 */
	function getSortOrder()
	{
		...
		if(isset($_REQUEST['sorder']))
			...
			$sorder = (isset($_SESSION['RELSETTINGS_SORT_ORDER'])?($_SESSION['RELSETTINGS_SORT_ORDER']):($this->default_sort_order));
		$log->debug("Exiting getSortOrder() method ...");
		return $sorder;
	}

	/**	Function used to get the order by value for RELSETTINGS listview
	 *	@return string	$order_by  - first check the $_REQUEST['order_by'] if request value is empty then check in the $_SESSION['RELSETTINGS_ORDER_BY'] if this session value is empty then default order by will be returned.
	 */
	function getOrderBy()
	{
		global $log;
		...
		if (isset($_REQUEST['order_by']))
			...
			$order_by = (isset($_SESSION['RELSETTINGS_ORDER_BY'])?($_SESSION['RELSETTINGS_ORDER_BY']):($this->default_order_by));
	...
	*/
	...
	{
		/*
		$where_clause = "and ec_relsettings.relsettingname like '%".trim($this->column_fields['relsettingname'])."%'";
		$query = "SELECT * FROM ec_relsettings  where deleted=0 $where_clause";
	...
	function get_contacts($id)
	{
		...
		return GetRelatedList('Relsettings','Contacts',$focus,$query,$button,$returnset);
	}
